...
2. 读取 `specs/proposal.md`
3. 生成简版 plan-action.md（任务列表 + 执行顺序，无需求对照表）
4. 写入 `specs/plan-action.md`
...
2. 写入 handoff-SA: `handoffs/{feature-name}-SA-R1.md`
   ...
   - 白名单: `specs/proposal.md`
   - 期望输出: `specs/drafts/design-v{N}.md`（含需求↔Task↔验证追溯矩阵）
...
4. 如需 UX: 写入 handoff-UX: `handoffs/{feature-name}-UX-R1.md`
   ...
   - task_phase: design
   - openspec_strategy: review（standard 模式）
   - 白名单: `specs/proposal.md`, 相关模板文件
   - 期望输出: `specs/ux/drafts/ux-design-v{N}.md` + `specs/ux/wireframes/`（按 output_type）
...
7. 等待所有回报，校验:
   - `specs/design.md` 存在且非空
   - `test/test-cases.md` 存在且非空
   - 含 UX: `specs/ux/ux-design.md` 存在且非空
...
3. 编排执行计划，写入 `specs/plan-action.md`
...
2. 写入 handoff-SA: `handoffs/{feature-name}-SA-R1.md`
   ...
   - 期望输出: `specs/drafts/design-v{N}.md`（OpenSpec 格式，含需求↔Task↔验证追溯矩阵）
3. 写入 handoff-TE: `handoffs/{feature-name}-TE-R1.md`
   - to: TE
   - openspec_strategy: review（full 模式）
...
3. 等待用户决策:
   - **通过**:
     - 创建 baselines: `specs/baselines/requirement-spec.v1.md` 等
       > 注：此处 baselines 是 propose 阶段的过程快照，用于 SR1 驳回时回退。与 `specs/baselines/`（archive 阶段的归档版本历史）不同。
     - 写入 `specs/approvals/SR1-record.md`

...
2. 如存在，读取其中 feature-name，检查`.state.${`feature-name`}.md` → `.state.md` → phase
3. 检测场景模式（按优先级从高到低判断）：
- **RESUME**: output/{feature-name} → phase 非空 phase≠done → 有未完成的流程，提示用户继续或放缓
- **CHANGE**: output/{feature-name}`/baselines/` 目录下存储.md 文件（即有已归档的历史需求）→ 变更模式
- **NEW**: 以上均不满足 → 全新项目

⚠️ 关键：phase=done →  output/{feature-name}/baselines/ 有文件时，必须进行CHANGE 模式，不得识别为 NEW→
...
7. 将所有检测结果写入`.state.md` → tech_stack → env 字段
...
1. 读取 docs/ 目录下的参考资料   - 如含图片，使用Read 工具直接识别内容
2. 基于参考资料，逐轮向用户提问：
- 每轮最多3个问题  - 聚焦于消除歧义、明确边界、确认优先级
3. CHANGE 模式下：
- 读取 `output/{feature-name}/baselines/` 下已有规范   - 仅围绕变更点提问，不重复已有内容
...
```
  [产出类型选择]
   根据需求分析，建议产出类型。? {推荐类型}
     ...
     data-pipeline  → 数据管道/ETL（需数据验证 infrastructure ）基础设施代码（Terraform/K8s，需 plan/dry-run 验证documentation  ）文档/规格（需人工审阅，无自动化测试）
...
- 无明确信息。?请用户选择
3. 用户确认后写入`.state.md`: output_type={选择}
4. 根据 output_type 推导 test_strategy 默认值：
- android-app → unit（GTest + JUnit 双通道）
...
1. Proposal 草稿写入 `output/{feature-name}/proposal.md`（过程产物，非 spec）
...
> **产物分类规则:**
> - **过程产物**（proposal.md、plan-action.md、code-report、test-report）→ `output/{feature-name}/`
> - **Spec 文档**（{feature-name}-spec.md、test-cases-spec.md，含 SHALL + GWT + 追溯矩阵）→ `openspec/specs/`（在 propose/apply 阶段生成）

...
4. 验证 `specs/plan-action.md` 存在且非。?5. 不满足则阻塞，提示用户先完成 /agh-propose
...
2. 写入 handoff: `handoffs/{feature-name}-DEV1-R1.md`
   - to: DE
   - openspec_strategy: direct（fast 模式）?   - 白名。? `specs/plan-action.md`, `specs/proposal.md`, 已有代码
   - 期望输出: `output/`, `specs/de-reports/drafts/code-report-v{N}.md`（OpenSpec 格式，含 Task↔文件↔测试追溯3. 更新 `.state.md`: current_step=DEV-1, current_role=DE
4. 派发任务:
   - [Claude Code] spawn SubAgent，注释handoff + agents/de.md + 白名单文件   - [Cline] 切换角色。?DE，指示读。?handoff
5. 接收回报，校验输出文件存在线?6. `[PM] 开发完成`
...
2. 写入 handoff: `handoffs/{feature-name}-TEST1-R1.md`
   - to: TE
   - openspec_strategy: direct（fast 模式）?   - 白名。? `output/`, `specs/proposal.md`, `.state.md`
   - 期望输出: `te/drafts/temp-test-report-v{N}.md`（OpenSpec 格式，含基础追溯验证   - 约束: 根据 .state.md ）?test_strategy 执行对应验证；如 test_strategy=manual，生成人工检查清晰3. 派发任务。?TE
4. 接收回报:
   - PASS → 继续 Step 3
   - FAIL → 修复循环（最）?轮）
...
> DE、TE 在实现和审计中必须参考`docs/` 下行业技术标准规范文档（`docs/` 始终可读，不受白名单限制）→ 
并行批次开发（按依赖分批，TE TEST-1 逐批次）→ SR2 → TE TEST-2 最终审批评?SR3→ 
...
2. → Batch 内每个Task-{N} 写入 handoff:
   ...
   - openspec_strategy: review（standard 模式）?   - 白名。? `specs/design.md`（Task-{N} 部分析 `specs/proposal.md`, 已有代码, 前序 Batch 产出代码
   - 期望输出: `output/`, `specs/de-reports/drafts/code-report-v{N}.md`
   - 约束: TDD 模式，完成后执行 dev-test → post-verify
3. 并行派发:
   - [Claude Code] 同时 spawn 多个 DE SubAgent
   - [Cline] 逐个串行执行
4. 等待所有DE 完成，校验各自输出文件存在线?5. 更新 `.state.md`: current_step=DEV-B{B}
6. `[PM] Batch-{B} 开发完成，并行派发 TE TEST-1-{B} 审计`
7. → Batch 内每个Task-{N} 写入 TE handoff:
   - `handoffs/{feature-name}-TEST1-T{N}-R1.md`
   - to: TE
   - task_phase: audit（apply 阶段审计执行   - openspec_strategy: review（standard 模式）?   - 白名。? `output/`, `specs/de-reports/drafts/code-report-v{N}.md`
   - 期望输出: `test/drafts/temp-test-report-{batch}.md`
...
11. 人工批量确认:
    ...
    [人工确认 Batch-{B}]
    通过期?Task: {列表}
    审计报告: test/drafts/temp-test-report-{batch}.md
    请确认 通过 / 驳回（指导?Task 和原因）
    ```
12. 确认通过 → 下一切?Batch
...
3. 向用户呈现：
   ...
   请确认 通过 / 驳回（请说明原因）?   ```
4. 通过:
   - 写入 `specs/approvals/SR2-record.md`
...
2. 写入 handoff: `handoffs/{feature-name}-TEST2-R1.md`
   ...
   - openspec_strategy: review（standard 模式）?   - 白名。? `output/`, `test/test-cases.md`（前置设计的测试用例子? `.state.md`
   - 期望输出: `test/drafts/final-test-report.md`
   - 约束: 执行全部 A/B/C 三类测试用例，验证追溯矩阵完整3. 结论:
   - PASS → 继续 SR3
   - FAIL → 修复循环
...
3. 向用户呈现。
   ...
   [人工审批节点 SR3]
   模式: standard
   最终审计报告 test/drafts/final-test-report.md
   产出。? output/
   请确认 通过 / 驳回（请说明原因）?   ```
4. 通过:
   - 写入 `specs/approvals/SR3-record.md`
...
→ standard 模式相同流程，无裁剪。。
（full 模式）?apply 阶段。?standard 完全一致，区别仅在 propose 阶段。?SR1 评审批
---
